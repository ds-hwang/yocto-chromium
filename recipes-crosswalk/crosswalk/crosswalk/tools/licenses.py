...
Commands:
  ...
  license  print license tags for bitbake
...
def GenerateLicenseTags():
    """Generate LICENSE_path/to/component tags for bitbake"""

    if len(sys.argv) is not 3:
      print 'usage: licenses.py license path-to-license-database-dir'
      return False
    ...
    for path in sorted(third_party_dirs):
        ...
        license_metadata = metadata['License'].replace(' ', '-')
        license_original = open(metadata['License File'], 'rb').read()

        md5_hash = hashlib.md5()
        md5_hash.update(license_original)
        license_original_md5 = md5_hash.hexdigest()

        candidates = []

        if license_metadata in license_database:
            candidates.append((1.0, license_metadata))
        else:
            for file in license_database:
                license_template = open(os.path.join(sys.argv[2], file), 'rb').read()

                s = difflib.SequenceMatcher(lambda x: x == " " or x == "\n",
                        license_original, license_template)

                candidates.append((round(s.ratio(), 2), file))

        candidates.sort()
        confidece = candidates[-1][0]
        license_guess = candidates[-1][1]

        license = ""
        if confidece == 1.0:
            license = license_guess
        else:
            license = 'FIXME: ' + license_metadata + ' -> ' + license_guess

        print str(confidece) + ' LICENSE_' + path + ' = "' + license + '"'
        print 'CRC file://${S}/' + os.path.relpath(metadata['License File']) + ';md5=' + license_original_md5 + ' \\'
